...
//   const role = 'organizer'; // or 'admin'
    ...
    const [eventWithSecretCode, setEventWithSecretCode] = useState(null);

    // const [events, refetch , isLoading] = role === 'admin' ? useEvent({num:0,status:'all'}) : useEvent({num:0,status:'all',organizerEmail:user?.email})
    const [filteredEvents, refetch, isLoading] = role === 'admin' && !showAdminEventsOnly 
    ? useEvent({
        search: searchTerm,
        status: statusFilter,
    ...
    const [totalEvents, isCountLoading] = role === 'admin' ? useEventCount() : useEventCount({ organizerEmail: user?.email });
    // console.log(events);
...
// }, [openDropdownId]);
  ...
  const getAvailableActions = (event) => {
    ...
    if (event.status === 'completed') {
      return [...baseActions, 'duplicate', 'reviews'];
    }
    ...
    if (isOwner) {
      return [...baseActions, 'edit', 'volunteers', 'cancel', 'secretCode'];
    }
    
    if (isAdmin) {
      return [...baseActions, 'volunteers', 'cancel', 'feature', 'secretCode'];
    }
  ...
  const ActionButton = ({ action, event, onClick }) => {
    const actionConfig = {
    view: { icon: Eye, label: 'View', className: 'text-gray-600 dark:text-gray-300 hover:text-[#FF6B00] dark:hover:text-[#FF6B00]' },
    edit: { icon: Edit, label: 'Edit', className: 'text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-500' },
    volunteers: { icon: Users, label: 'Volunteers', className: 'text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-500' },
    cancel: { icon: XCircle, label: 'Cancel Event', className: 'text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-500' },
    feature: { icon: Star, label: event.featured ? 'Unfeature' : 'Feature', className: 'text-yellow-600 dark:text-yellow-400 hover:text-yellow-700 dark:hover:text-yellow-500' },
    duplicate: { icon: Copy, label: 'Duplicate', className: 'text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-500' },
    secretCode: { icon: Key, label: 'Secret Code', className: 'text-yellow-600 dark:text-yellow-400 hover:text-yellow-700 dark:hover:text-yellow-500' },
    reviews: { icon: MessageSquare, label: 'Reviews', className: 'text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-500' },
    // report: { icon: Flag, label: 'Report', className: 'text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-500' },
    // moderate: { icon: Flag, label: 'Moderate', className: 'text-orange-600 dark:text-orange-400 hover:text-orange-700 dark:hover:text-orange-500' }
};
    ...
    if(action === 'view'){
      ...
      if(!isEditable){
        return <button onClick={() => {toast.error("Cannot edit this event, less than 24 hours remaining.", { position: 'top-right', style: {
      ...
      return <Link to={`/dashboard/reviews/${event?._id}`} className={`p-1 rounded hover:bg-gray-100 transition-colors ${config.className}`}
        title={config.label}><Icon className="w-4 h-4" /></Link>
    }
  ...
  const loadingDesign = <>
      {/* Desktop Table Loading Skeleton */}
          <div className="hidden lg:block overflow-x-auto bg-white dark:bg-gray-800 rounded-lg shadow-sm dark:shadow-md animate-pulse">
            <table className="w-full">
              <thead className="bg-gray-100 dark:bg-gray-700 h-14 border-b border-gray-200 dark:border-gray-600">
                <tr>
                  <th className="text-left pl-6 py-3">
          ...
          <div className="lg:hidden">
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="p-4 border-b border-gray-200 dark:border-gray-700 last:border-b-0 animate-pulse">
  ...
  return (
    <div className="px-4 pb-4 sm:p-4 lg:p-6 xl:px-12 bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      {/* Search and Filters */}
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm dark:shadow-md mb-6 p-4">
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full outline-none pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:ring-2 focus:ring-[#FF6B00] focus:border-transparent"
            />
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full outline-none border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#FF6B00] focus:border-transparent bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300"
                >
              ...
              <div className="flex items-end">
                <button
                  ...
                  className="w-full px-4 py-2 rounded-lg hover:bg-gray-200 transition-colors bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 dark:hover:bg-gray-600"
                >
                  Clear Filters
                </button>
              </div>
        ...
        <div className="mb-4">
          <div className="flex items-center">
            <input
              ...
              onChange={(e) =>{
              setShowAdminEventsOnly(e.target.checked);
              refetch();
            }}
              className="h-4 w-4 text-[#FF6B00] cursor-pointer border-gray-300 rounded focus:ring-[#FF6B00]"
            />
            <label htmlFor="show-my-events" className="ml-2 block text-sm text-gray-900 dark:text-gray-100">
              Show only my events
            </label>
          </div>
        </div>
      ...
      <div className=" overflow-hidden">
        ...
        <>{filteredEvents?.length === 0 ? (
          ...
          <>
            {/* Desktop Table View */}
            <div className="hidden lg:block overflow-x-auto bg-white dark:bg-gray-800 rounded-lg shadow-sm dark:shadow-md">
              <table className="w-full">
                <thead className="bg-gray-100 dark:bg-gray-700 h-14 border-b border-gray-200 dark:border-gray-600">
                  <tr className=''>
                    <th className="text-left cursor-default pl-6 pr-36 py-3 tracking-wider text-sm font-medium text-gray-700 dark:text-gray-300">Event</th>
                    {role === 'admin' && (
                      <th className="text-left cursor-default w-60 px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-300">Organizer</th>
                    )}
                    <th className="text-left cursor-default px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-300">Status</th>
                    <th className="text-left whitespace-nowrap cursor-default pl-6 pr-20 py-3 text-sm font-medium text-gray-700 dark:text-gray-300">Date & Time</th>
                    <th className="text-left cursor-default px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-300">Volunteers</th>
                    <th className="text-left cursor-default px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-300">Views</th>
                    <th className="text-center cursor-default px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-300">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredEvents?.map((event) => (
                    <tr key={event._id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      <td className="px-4 py-4 flex flex-col justify-center gap-2 text-left">
                        <div className="text-sm font-medium text-gray-900 dark:text-gray-100 line-clamp-2">{event.title}
                        <span className="bg-[#FF6B00] bg-opacity-10 dark:bg-opacity-20 ml-2 text-[#FF6B00] px-2 py-0.5 rounded text-xs font-medium">{event.category}</span></div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">{event.location}</div>
                    </td>
                      {role === 'admin' && (
                        <td className="px-4 py-4">
                          <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{event.organizerName}</div>
            ...
            <div className="lg:hidden">
              {filteredEvents?.map((event) => (
                <div key={event._id} className="p-4 border-b border-gray-200 dark:border-gray-700 last:border-b-0">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center space-x-3 flex-1">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-2 mb-2">
                          <span className="bg-[#FF6B00] bg-opacity-10 dark:bg-opacity-20 text-[#FF6B00] px-2 py-0.5 rounded text-xs font-medium">{event.category}</span>
                          {getStatusBadge(event.status)}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-sm text-gray-600 dark:text-gray-400">{event.views || 0}</span>
                      </div>
                    </div>
                    <div className="flex items-center space-x-1 relative">
                      {getAvailableActions(event).slice(0, 2).map((action) => (
                        <ActionButton
                          key={action}
                          action={action}
                          event={event}
                      ...
                      {getAvailableActions(event)?.length > 2 && (
                        <div>
                        <button ref={buttonRef}
                onClick={() => {
                    const newValue = openDropdownId === event._id ? null : event._id;
                    setOpenDropdownId(newValue);
                }}  className="dropdown-toggle-btn p-1 rounded hover:bg-gray-100 dark:bg-gray-900 transition-colors text-gray-600 dark:text-gray-400">
                          <MoreVertical className="w-4 h-4 off" />
                        </button>
                        {openDropdownId===event._id && (
                            <div ref={dropdownRef} className="absolute right-0 bottom-8 mt-2 w-max flex items-center py-0 px-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-10">
                                {getAvailableActions(event).slice(2).map((action) => (
                                    <div key={action} className="px-2 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors cursor-pointer">
                                        <ActionButton
                                            key={action}
                                            action={action}
                                            event={event}
      ...
      <div className="mt-6 text-center text-sm text-gray-500 dark:text-gray-400">
        {!isLoading && `Showing ${filteredEvents?.length} of ${totalEvents} events`}
      </div>
      <SecretCodeModal
        isOpen={showSecretCodeModal}
        onClose={() => setShowSecretCodeModal(false)}
        event={eventWithSecretCode}
      />
      <CancelEventModal
