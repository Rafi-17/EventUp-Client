...
const EventCard = ({ event }) => {
    ...
    return (
        <div className="bg-white rounded-lg shadow-xl overflow-hidden transform transition-transform duration-300 hover:scale-[1.03] hover:shadow-2xl">
            {/* Image section */}
            <div className="relative h-48 sm:h-56 w-full">
                ...
                />
                <span className={`absolute top-4 right-4 px-3 py-1 text-sm font-semibold rounded-full ${event?.status==='completed' ? 'text-gray-800 bg-gray-100' : 'text-green-800 bg-green-100'}`}>
                    {event.status.charAt(0).toUpperCase() + event.status.slice(1)}
                </span>
            </div>
            
                <div className="mb-2">
                    <span className="px-2 py-1 text-sm font-semibold rounded-full text-blue-800 bg-blue-100">
            ...
            <div className="px-4 pb-4 sm:px-6 sm:pb-6">
                <div className="h-12 sm:h-16 flex flex-wrap items-start">
                    <h3 className="text-xl sm:text-2xl font-bold text-gray-800 leading-tight line-clamp-2">
                        {event.title}
                    </h3>
                </div>
                ...
                <div className="flex items-center text-gray-600 mb-1">
                    ...
                    <p className="font-semibold text-sm">{formatMinutesToDuration(event?.duration) || "Not Fixed"}</p>
                </div>
                    ...
                    <p className="font-semibold text-sm">
                        {volunteersNeeded ? (
                            <span>
                                {vacanciesLeft > 0 ? (
                                    <span>
                                        Vacancies Left: <span className="text-[#FF6B00]">{vacanciesLeft}</span>
                                    </span>
                                ) : (
